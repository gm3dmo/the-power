...
def main(args):
    ...
    p = Path('test-data/workflow-packages.yml_')
    json_file = f"""tmp/workflow-packages.json"""
    filename_in_repo = f"""workflow-packages.yml"""
    with open(p, 'rb') as ct:
       t = {}
       chapter_content = ct.read()
       chapter_base64 = base64.encodebytes(chapter_content)
       t["message"] = f"""A packages workflow file."""
       t["committer"] = {}
