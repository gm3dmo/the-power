...
def make_request(i, args, http, token, counter, lock, total_time, batch_start_time):
    headers = {
        ...
        "User-Agent" : f"pwr-content-creator-{i}"
    }

    repo_name = f"""{args.repo_prefix}-{i:07}""" 

    # Read the content from create-commit-to2c.json
    with open('create-commit-to2c.json', 'r') as file:
        params = json.load(file)

    params = json.dumps(params)

    url =  f"""{args.url}/repos/{args.org}/{repo_name}/contents/to2c.txt"""
    print(url)
    #print(params)
    ...
    try:
        ...
        response = http.request('PUT', url, body=params, headers=headers)
        ...
        if response.status == 201:
            ...
            raise Exception(f"Failed to create content on repo {repo_name}, status code: {response.status}")
    except Exception as e:
        request_id = response.headers.get('x-github-request-id', 'N/A') if response else 'N/A'
        error_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print(f"[{error_time}] Error creating content on {repo_name}: {e}, x-github-request-id: {request_id}", file=sys.stderr)
